#!/usr/bin/env python3
"""
Optimized 309-digit Gap Hunter using 2048-bit CGBN

NOTE: Using 2048-bit container for 10^309:
- 10^309 requires 1027 bits
- 2048-bit container provides ample headroom (up to 617 digits)
- Fixed bugs: even number check, shift vs rotate

Optimizations:
1. Single-instance GPU access (no competing processes)
2. CPU worker pool for pre-sieving (balanced ~16% survival)
...
# Paths
CGBN_BINARY = "/tmp/cgbn_2048_primality"
...
def int_to_limbs_2048(n):
    """Convert Python int to 64 uint32 limbs (little-endian, 2048-bit)"""
    limbs = []
    for _ in range(64):
...
def cgbn_batch_test(candidates, gpu_lock=None):
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix='.bin') as f:
            input_file = f.name
            for n in candidates:
                limbs = int_to_limbs_2048(n)
                f.write(struct.pack('<64I', *limbs))
...
class PipelinedGapHunter:
    ...
    def run_pipelined(self):
        ...
        """
        print(f"\n{'='*70}")
        print(f"Optimized 309-Digit Gap Hunter (2048-bit CGBN + Wheel Sieve)")
        print(f"{'='*70}")
        print(f"Base: 10^309")
        ...
        print(f"Expected filter rate: ~85% eliminated by CPU")
        print()
    ...
    def save_results(self, total_time):
        ...
        filename = RESULTS_DIR / f"gaps_309digit_optimized_{timestamp}.md"

        s = self.stats
        filter_reduction = (1 - s['candidates_after_sieve'] / s['candidates_generated']) * 100

        with open(filename, 'w') as f:
            f.write(f"# 309-Digit Prime Gap Results (2048-bit CGBN)\n\n")
...
def main():
    ...
    # Base at 10^309 (requires 2048-bit CGBN)
    base = 10**309
    if base % 2 == 0:
        base += 1
