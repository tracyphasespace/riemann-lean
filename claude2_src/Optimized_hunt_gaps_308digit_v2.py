...
2. CPU worker pool for pre-sieving (eliminates ~90% of candidates)
3. Trial division pre-filter (eliminates ~30% more)
4. Pipelined batch processing (CPU prepares while GPU computes)

Expected speedup: 10-30x over naive approach
"""
...
SIEVE_PRIMES_LIMIT = 10000      # Sieve with primes up to this
TRIAL_DIVISION_LIMIT = 100000   # Trial division with primes up to this
CPU_WORKERS = max(1, cpu_count() - 2)  # Leave cores for GPU management
GPU_BATCH_SIZE = 100000         # Candidates per GPU batch
...
def presieve_candidate(n):
    ...
    Skip 2 since we only test odd numbers.
    """
    for p in SIEVE_PRIMES[1:]:  # Skip 2
        if n % p == 0:
            return False
    return True
...
class PipelinedGapHunter:
    ...
    def generate_batch(self, batch_idx):
        """Generate a batch of odd candidates"""
        offset = batch_idx * self.batch_size * 2  # *2 for odd numbers
    ...
    def process_batch(self, candidates, batch_idx, total_batches):
        ...
        filtered = parallel_filter_candidates(candidates, use_trial_division=True)
    ...
    def run_pipelined(self):
        ...
        """
        print(f"\n{'='*70}")
        print(f"Optimized 308-Digit Gap Hunter (Pipelined)")
        print(f"{'='*70}")
        print(f"Base: 10^308")
        print(f"Total candidates: {self.total_candidates:,}")
        print(f"Batch size: {self.batch_size:,}")
        print(f"CPU workers: {CPU_WORKERS}")
        print(f"Sieve primes: {len(SIEVE_PRIMES)} (up to {SIEVE_PRIMES_LIMIT})")
        print(f"Trial division primes: {len(TRIAL_PRIMES)} (up to {TRIAL_DIVISION_LIMIT})")
        print(f"Expected filter rate: ~90% eliminated by CPU")
        print()
    ...
    def save_results(self, total_time):
        ...
        with open(filename, 'w') as f:
            ...
            f.write(f"1. Single-instance GPU access (no competition)\n")
            f.write(f"2. CPU pre-sieve with {len(SIEVE_PRIMES)} primes (up to {SIEVE_PRIMES_LIMIT})\n")
            f.write(f"3. Trial division with {len(TRIAL_PRIMES)} primes (up to {TRIAL_DIVISION_LIMIT})\n")
            f.write(f"4. Pipelined batch processing ({CPU_WORKERS} CPU workers)\n\n")
