...
- **Axioms**: 5 core (geometric_dominance, hard_case_7/15/27/31)
- **Certificates**: 28 of 32 residue classes proven via native_decide
- **Hard cases**: 4 classes covered via axioms
-/
...
Justification:
- If Axioms.trajectoryDescends n k = true, the standard trajectory has a value m with 0 < m < n
- The compressed T trajectory reaches the same values (or gets ahead)
- Therefore T^j(n) < n for some j ≤ k
-/
axiom standard_to_compressed_descent (n : ℕ) (hn : 1 < n)
