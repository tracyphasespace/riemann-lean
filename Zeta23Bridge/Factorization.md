...
- **F2b: the equality-grade Abel layer.** The remaining §5 inputs are asymptotic *equalities*, not bounds — the integrated second moment Σf²/n·(L−log n) = L³/6 + O(L²) and the g-weighted diagonal — and these genuinely need the Abel/integral route from `mertens_energy` (dyadic descent cannot produce main terms). Adapting [Z23]'s `Zeta23.Cheb` machinery (currently Λ-hardcoded) to abstract interface-f.
